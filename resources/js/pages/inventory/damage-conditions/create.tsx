...
export default function Create() {
    const { data, setData, post, processing, errors, reset } = useForm({ condition: '', order_column: 0 });

    const submit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        post(route('inventory.damage-conditions.store'), {
            onSuccess: () => reset(),
        });
    };
    ...
    return (
        <AppSidebarLayout breadcrumbs={breadcrumbs}>
            <Head title="破損状態 新規作成" />
            <div className="py-12">
                <div className="mx-auto max-w-2xl sm:px-6 lg:px-8">
                    <form onSubmit={submit}>
                        <Card>
                            <CardHeader>
                                <CardTitle>新しい破損状態を作成してください</CardTitle>
                            </CardHeader>
