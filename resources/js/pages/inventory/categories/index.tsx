...
export default function Index({ categories: initialCategories = [], queryParams = {} }: any) {
    ...
    const confirmAndDelete = async (cat: any) => {
        ...
        try {
            ...
            const res = await fetch(route('inventory.categories.destroy', cat.id), {
                method: 'DELETE',
            ...
            // If server returned JSON with error/success, show toast accordingly
            ...
            // Refresh the index to reflect deletion (and to pick up any server flash if necessary)
            router.get(route('inventory.categories.index'), {}, { preserveState: false, preserveScroll: false });
        } catch (err) {
            console.error(err);
